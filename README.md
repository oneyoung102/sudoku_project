...
1. 자신의 최고 점수에 따라 난이도 상승,하강 여부가 경정
2. 난이도별 보드 크기를 담은 튜플에서 랜덤으로 크기 설정(2x2 ~ 5x5), 보드 크기가 10 이상이면 숫자대신 알파벳으로 출력
3. 난이도별 보드 크기에 따른 빈칸 개수를 담은 튜플에서 랜덤으로 오차 범위 내에서 빈칸 개수 설정
  > **오차범위 계산식**
  > ```python
